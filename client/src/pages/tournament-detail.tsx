...
function AdminMatchStats({ matchId, playerAName, playerBName }: { matchId: number; playerAName: string; playerBName: string }) {
  ...
  const handleScoresToggle = async (e: React.MouseEvent) => {
    e.stopPropagation();
      ...
      {scoresOpen && (
        <div className="mt-2 border-t pt-2">
          ...
          {!loading && hasLegHistory && (
            <LegScoresContent legHistory={notes.legHistory} playerAName={playerAName} playerBName={playerBName} />
          )}
          {!loading && !hasLegHistory && (
            <p className="text-xs text-muted-foreground text-center py-1">No score breakdown available</p>
          )}
        </div>
      )}
    </div>
