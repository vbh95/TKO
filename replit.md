...
Tournament collaborator system: Tournament owners can invite other TKO users as collaborators via their email address. Collaborators get full admin access (manage players, score matches, use tablets, manage board sessions, configure sharing) except they cannot delete the tournament, reset it, or manage collaborators themselves. `tournament_collaborators` table stores `(tournamentId, userId, invitedByUserId)` with unique constraint on `(tournamentId, userId)`. `isAuthorizedForTournament(tournament, userId)` helper checks owner OR collaborator for most routes; reset endpoint is owner-only. `GET /api/tournaments` returns all owned + collaborated tournaments with `isOwner`/`isCollaborator`/`ownerName`/`collaboratorCount` flags. `GET /api/tournaments/:id` also returns `ownerName`/`isOwner`/`isCollaborator`. Collaborator endpoints: `GET/POST/DELETE /api/tournaments/:id/collaborators`. Unified settings popup (gear icon) on both dashboard cards and tournament detail page contains: rename, league, tablet scoring, collaborators (owner-only), format settings (owner-only), delete (owner-only). Dashboard shows blue "COLLABORATOR" badge on collaborated tournaments. Cards show owner name and collaborator count chip. Create Tournament page has optional Collaborators section (emails invited post-creation). Real-time sync via WebSocket `tournament:{id}` room.
