...
async function promoteGroupToKnockout(params: PromoteGroupParams) {
  ...
  if (completedGroupIdx < firstRoundMatches.length) {
    ...
    if (lastPlacePlayer) {
      await storage.updateMatch(firstRoundMatches[completedGroupIdx].id, {
        scorerId: lastPlacePlayer.id,
        scorerName: lastPlacePlayer.name,
      } as any);
    }
  }
...
async function backfillKnockoutScorers() {
  try {
    ...
    for (const tournament of allTournaments) {
      ...
      const allQfMatches = knockoutMatches
        .filter(m => m.roundKey === 'QF')
        .sort((a: any, b: any) => a.order - b.order);

      for (let gi = 0; gi < groupsList.length && gi < allQfMatches.length; gi++) {
        const qf = allQfMatches[gi];
        if (qf.scorerId !== null) continue;
        if (!qf.playerAId && !qf.playerBId) continue;
        ...
        await storage.updateMatch(qf.id, { scorerId: lastPlace.id, scorerName: lastPlace.name } as any);
      }
...
export async function registerRoutes(
  ...
  app.get(api.tournaments.get.path, isAuthenticated, async (req, res) => {
    ...
    // Auto-repair TP_QF slots if TP is enabled and any are still unpopulated.
    // Route-level guard uses already-loaded data to avoid extra DB reads when no repair needed.
    try {
      const tSettings = tournament.settings as TournamentSettings | undefined;
      const tpEnabled = !!(tSettings?.enableThirdPlaceBracket && tSettings?.groupScheduleMode === 'board_rotation');
      const tpNeedsRepair = tpEnabled && matches.some(m =>
        m.stage === 'KNOCKOUT' && m.roundKey.startsWith('TP_') && (!m.playerAId || !m.playerBId)
      );
      if (tpNeedsRepair) {
        const repaired = await repairTpBracket(id, tSettings);
        if (repaired) {
  ...
  app.get('/api/scorer/board-data', isBoardAuthenticated, async (req: any, res) => {
    try {
      ...
      const [playersList, groupsList, matchesRaw, allMemberships] = await Promise.all([
      ...
      // Auto-repair TP_QF slots if TP is enabled and any are still unpopulated.
      // Route-level guard uses already-loaded matches to skip extra DB calls when repair isn't needed.
      let allMatches = matchesRaw;
      try {
        const tSettings = tournament.settings as TournamentSettings | undefined;
        const tpEnabled = !!(tSettings?.enableThirdPlaceBracket && tSettings?.groupScheduleMode === 'board_rotation');
        const tpNeedsRepair = tpEnabled && allMatches.some(m =>
          m.stage === 'KNOCKOUT' && m.roundKey.startsWith('TP_') && (!m.playerAId || !m.playerBId)
        );
        if (tpNeedsRepair) {
          const repaired = await repairTpBracket(tournamentId, tSettings);
          if (repaired) {
            allMatches = await storage.getMatchesByTournamentId(tournamentId);
          }
        }
      } catch (err) {
        console.error('[TP repair] Failed to repair TP bracket on board-data load:', err);
      }
